...
// PowerMonitorDeployer deploys Kepler DaemonSet and ConfigMap for the given PowerMonitorInternal
...
func (r PowerMonitorDeployer) Reconcile(ctx context.Context, cli client.Client, s *runtime.Scheme) Result {
	additionalConfigs, err := r.readAdditionalConfigs(ctx, cli)
	if err != nil {
		return Result{Action: Stop, Error: fmt.Errorf("error creating kepler config: %w", err)}
	}

	cf := powermonitor.NewPowerMonitorConfigMap(components.Full, r.Pmi, additionalConfigs...)
	ds := powermonitor.NewPowerMonitorDaemonSet(components.Full, r.Pmi)
	powermonitor.MountConfigMapToDaemonSet(ds, cf)

	// Update the ConfigMap in the cluster
	result := Updater{Owner: r.Pmi, Resource: cf}.Reconcile(ctx, cli, s)
	if result.Error != nil {
		return result
	}

	return Updater{Owner: r.Pmi, Resource: ds}.Reconcile(ctx, cli, s)
}
...
func (r PowerMonitorDeployer) readAdditionalConfigs(ctx context.Context, c client.Client) ([]string, error) {
	...
	for _, ref := range cfmRefs {
		...
		if err := c.Get(ctx, types.NamespacedName{Namespace: ns, Name: ref.Name}, cfm); err != nil {
			...
			return nil, fmt.Errorf("failed to get ConfigMap %q in %q namespace: %w", ref.Name, ns, err)
		}
